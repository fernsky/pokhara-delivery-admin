...
import { Image, X, FileText, Video, FileImage, Fuel } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { api } from "@/trpc/react";
import { useRouter } from "next/navigation";
import { toast } from "sonner";

interface PetrolPumpMediaProps {
  petrolPumpId: string;
  media: any[];
}

export function PetrolPumpMedia({ petrolPumpId, media }: PetrolPumpMediaProps) {
  ...
  const handleSetPrimary = (mediaId: string) => {
    setPrimaryMedia({
      mediaId,
      entityId: petrolPumpId,
      entityType: "PETROL_PUMP",
    });
  };
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            पेट्रोल पम्पको फोटोहरू
          </CardTitle>
        </CardHeader>
        <CardContent>
          {media.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              <Fuel className="h-20 w-20 mx-auto opacity-10 mb-4" />
              <p>कुनै फोटो फेला परेन</p>
              <p className="text-sm mt-2">
                पेट्रोल पम्पको फोटोहरू संलग्न गर्न सम्पादन पृष्ठमा जानुहोस्
              </p>
            </div>
          ) : (
