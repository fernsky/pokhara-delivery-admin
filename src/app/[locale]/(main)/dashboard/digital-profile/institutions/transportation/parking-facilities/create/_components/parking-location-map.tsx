...
import { MapIcon, RotateCcw, Compass, Square } from "lucide-react";
import "ol/ol.css";
...
export function ParkingLocationMap({
  ...
  useEffect(() => {
    ...
    if (initialAreaPolygon) {
      try {
        // For Polygon, first level is polygon array, second level is ring array, third level is coordinates
        ...
        console.error("Error adding polygon geometry:", error);
      }
    }
    ...
    if (initialLocationPoint) {
      ...
      if (!initialAreaPolygon) {
        // Only center on point if we don't have a polygon
  ...
  // Start drawing polygon for parking area boundary
  const startDrawingPolygon = () => {
    ...
    drawInteractionRef.current.on("drawend", (event) => {
      ...
      // Convert to GeoJSON format (note the nested array structure for polygons)
      const coordinates = geometry
        .getCoordinates()[0]
        .map((coord) => toLonLat(coord) as [number, number]);

      // Update state - note that polygon coordinates require an extra level of nesting
  ...
  const addModifyInteraction = () => {
    ...
    // Set up event handlers for both modify interactions
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="p-3 border-t bg-muted/20 text-xs">
        {isPointDrawing ? (
          <p>पार्किङ स्थानको बिन्दु राख्न नक्सामा क्लिक गर्नुहोस्</p>
        ) : isPolygonDrawing ? (
          <p>
            पार्किङ क्षेत्र कोर्न नक्सामा क्लिक गर्दै जानुहोस् र अन्त्यमा डबल
