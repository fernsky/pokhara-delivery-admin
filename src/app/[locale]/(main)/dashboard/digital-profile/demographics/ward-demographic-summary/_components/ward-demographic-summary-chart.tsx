...
interface WardDemographicSummaryData {
  id: string;
  wardNumber: number;
...
interface WardDemographicSummaryChartProps {
  data: WardDemographicSummaryData[];
}

export default function WardDemographicSummaryChart({
  data,
}: WardDemographicSummaryChartProps) {
  ...
  const barChartData = useMemo(() => {
    // Filter data if a specific ward is selected
    const filteredData =
      selectedWard !== "all"
        ? data.filter((d) => d.wardNumber === parseInt(selectedWard))
        : data;
    ...
    return filteredData
      ...
      .map((ward) => ({
        ...
        [`${selectedMetric}`]: ward[selectedMetric as keyof typeof ward] || 0,
        wardColor: `hsl(${ward.wardNumber * 30}, 70%, 50%)`,
      }));
  }, [data, selectedMetric, selectedWard]);

  // Prepare data for pie chart - only for gender breakdown
  const pieChartData = useMemo(() => {
    // For all wards combined or specific ward
    const filteredData =
      selectedWard !== "all"
        ? data.filter((d) => d.wardNumber === parseInt(selectedWard))
        : data;

    // Calculate totals across all filtered wards
  ...
  // Calculate ward-wise comparison for households vs population
  const householdComparisonData = useMemo(() => {
    // Filter data if a specific ward is selected
    const filteredData =
      selectedWard !== "all"
        ? data.filter((d) => d.wardNumber === parseInt(selectedWard))
      ...
      <CardContent>
        <Tabs defaultValue="bar" className="mt-2">
          <TabsList>
            ...
            <TabsTrigger value="comparison">तुलनात्मक चार्ट</TabsTrigger>
          </TabsList>
