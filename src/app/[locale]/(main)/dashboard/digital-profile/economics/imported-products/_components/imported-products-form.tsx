...
interface ImportedProductsFormProps {
  editId: string | null;
  onClose: () => void;
  existingData: any[];
}

export default function ImportedProductsForm({
  editId,
  onClose,
  existingData,
}: ImportedProductsFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const utils = api.useContext();

  const createMutation = api.profile.economics.importedProducts.add.useMutation(
    {
      onSuccess: () => {
        toast.success("नयाँ उत्पादन सफलतापूर्वक थपियो");
        utils.profile.economics.importedProducts.getAll.invalidate();
  ...
  const { data: editingData, isLoading: isLoadingEditData } =
    api.profile.economics.importedProducts.getById.useQuery(
      { id: editId || "" },
      { enabled: !!editId },
    );
