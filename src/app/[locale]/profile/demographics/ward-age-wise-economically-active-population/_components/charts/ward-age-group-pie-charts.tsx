...
interface WardAgeGroupPieChartsProps {
  wardNumbers: number[];
...
export default function WardAgeGroupPieCharts({
  wardNumbers,
  populationData,
  AGE_GROUP_NAMES,
  AGE_GROUP_COLORS,
}: WardAgeGroupPieChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {wardNumbers.map((wardNumber) => {
        const wardItems = populationData.filter(
        ...
        // Sort age groups for consistent presentation
        const ageGroupOrder = {
          "AGE_0_TO_14": 1,
          "AGE_15_TO_59": 2,
          "AGE_60_PLUS": 3
        ...
        const wardData = wardItems
          .map((item) => ({
            name: AGE_GROUP_NAMES[item.ageGroup] || item.ageGroup,
            value: item.population || 0,
            ageGroup: item.ageGroup,
            sortOrder: ageGroupOrder[item.ageGroup as keyof typeof ageGroupOrder] || 99
          }))
                  ...
                  >
                    {wardData.map((entry, index) => (
                      <Cell
                        ...
                        fill={
                          AGE_GROUP_COLORS[
                            entry.ageGroup as keyof typeof AGE_GROUP_COLORS
                          ] ||
