...
interface WardDrinkingWaterSourcePieChartsProps {
...
export default function WardDrinkingWaterSourcePieCharts({
  wardWiseData,
  WATER_SOURCE_GROUPS,
}: WardDrinkingWaterSourcePieChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {wardWiseData.map((wardData) => {
        ...
        const waterSourceGroups = Object.values(WATER_SOURCE_GROUPS)
          .map(group => ({
            name: group.name,
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {waterSourceGroups.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.color}
                      />
