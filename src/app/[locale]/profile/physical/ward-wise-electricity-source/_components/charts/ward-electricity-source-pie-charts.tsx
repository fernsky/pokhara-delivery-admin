...
interface WardElectricitySourcePieChartsProps {
...
export default function WardElectricitySourcePieCharts({
  wardWiseData,
  ELECTRICITY_SOURCE_CATEGORIES,
}: WardElectricitySourcePieChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {wardWiseData.map((wardData) => {
        ...
        const sourceCategories = Object.values(ELECTRICITY_SOURCE_CATEGORIES)
          .map(category => ({
            name: category.name,
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {sourceCategories.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.color}
                      />
