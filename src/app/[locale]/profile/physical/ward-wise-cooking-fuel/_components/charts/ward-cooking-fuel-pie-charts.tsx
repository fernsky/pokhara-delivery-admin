...
interface WardCookingFuelPieChartsProps {
...
export default function WardCookingFuelPieCharts({
  wardWiseData,
  COOKING_FUEL_CATEGORIES,
}: WardCookingFuelPieChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {wardWiseData.map((wardData) => {
        ...
        const fuelCategories = Object.values(COOKING_FUEL_CATEGORIES)
          .map(category => ({
            name: category.name,
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {fuelCategories.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.color}
                      />
