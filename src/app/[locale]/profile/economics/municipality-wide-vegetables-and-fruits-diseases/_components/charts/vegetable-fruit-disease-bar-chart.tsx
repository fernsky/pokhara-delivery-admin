...
interface VegetableFruitDiseaseBarChartProps {
  data: Array<{
    name: string;
    diseases: number;
    pests: number;
...
export default function VegetableFruitDiseaseBarChart({
  data,
  VEGETABLE_FRUIT_COLORS,
  vegetableFruitSummary,
}: VegetableFruitDiseaseBarChartProps) {
  // Custom tooltip component
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      const cropData = vegetableFruitSummary.find((c) => c.cropName === label);
