...
interface OilSeedPieChartProps {
  pieChartData: Array<{
    name: string;
    value: number;
    percentage: string;
...
export default function OilSeedPieChart({
  pieChartData,
  OIL_SEED_TYPES,
  OIL_SEED_COLORS,
  dataType,
      ...
      <PieChart>
        ...
        >
          {pieChartData.map((entry, index) => {
            // Find the original oil seed type key for color mapping
            const oilSeedKey =
              Object.entries(OIL_SEED_TYPES).find(
            ...
            return (
              <Cell
                key={`cell-${index}`}
                fill={
                  OIL_SEED_COLORS[oilSeedKey as keyof typeof OIL_SEED_COLORS] ||
