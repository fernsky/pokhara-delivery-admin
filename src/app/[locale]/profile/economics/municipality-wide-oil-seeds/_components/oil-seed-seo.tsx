...
export default function OilSeedSEO({
  ...
  const generateStructuredData = () => {
    ...
    const oilSeedStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${OIL_SEED_TYPES_EN[item.type] || item.type} in Pokhara Metropolitan City`,
      ...
      description: `${item.production.toFixed(2)} tonnes of ${OIL_SEED_TYPES_EN[item.type] || item.type} produced in Pokhara Metropolitan City (${((item.production / totalProduction) * 100).toFixed(2)}% of total production). Sales volume: ${item.sales.toFixed(2)} tonnes. Revenue: NPR ${item.revenue.toLocaleString()}.`,
    }));
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Oil Seed Types in Pokhara Metropolitan City (पोखरा महानगरपालिका)",
      description: `Oil seed production and sales statistics of Pokhara Metropolitan City with a total production of ${totalProduction.toFixed(2)} tonnes and sales of ${totalSales.toFixed(2)} tonnes (${((totalSales / totalProduction) * 100).toFixed(2)}% of production). The most common oil seed crop is ${mostProducedOilSeedEN} with ${mostProducedOilSeed?.production.toFixed(2)} tonnes (${mostProducedPercentage}%). Self-consumption represents ${selfConsumptionPercentage}% of total production. Total revenue from oil seed sales is NPR ${totalRevenue.toLocaleString()}.`,
      keywords: [
