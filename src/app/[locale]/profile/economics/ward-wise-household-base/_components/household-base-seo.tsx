...
export default function HouseholdBaseSEO({
  ...
  const generateStructuredData = () => {
    ...
    const baseTypeStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${BASE_TYPE_NAMES_EN[item.baseType] || item.baseType} in Pokhara Metropolitan City`,
      ...
      description: `${item.households.toLocaleString()} households in Pokhara Metropolitan City have ${BASE_TYPE_NAMES_EN[item.baseType] || item.baseType} (${((item.households / totalHouseholds) * 100).toFixed(2)}% of total households)`,
    }));
    ...
    return {
      ...
      "@type": "Dataset",
      name: "House Foundation Types in Pokhara Metropolitan City (पोखरा महानगरपालिका)",
      description: `House foundation data across ${wardNumbers.length} wards of Pokhara Metropolitan City with a total of ${totalHouseholds.toLocaleString()} households. The most common type is ${mostCommonTypeEN} with ${mostCommonType?.households.toLocaleString()} households (${mostCommonTypePercentage}%).`,
      keywords: [
