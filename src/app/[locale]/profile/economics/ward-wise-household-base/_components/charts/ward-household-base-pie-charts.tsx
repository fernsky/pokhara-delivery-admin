...
interface WardHouseholdBasePieChartsProps {
  wardNumbers: number[];
  baseData: Array<{
    id?: string;
    wardNumber: number;
...
export default function WardHouseholdBasePieCharts({
  wardNumbers,
  baseData,
  BASE_TYPE_NAMES,
  BASE_TYPE_COLORS,
}: WardHouseholdBasePieChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {wardNumbers.map((wardNumber) => {
        const wardItems = baseData.filter(
        ...
        // Sort base types for consistent presentation
        const baseTypeOrder = {
          "CONCRETE_PILLAR": 1,
          "CEMENT_JOINED": 2,
          "MUD_JOINED": 3,
        ...
        const wardData = wardItems
          .map((item) => ({
            name: BASE_TYPE_NAMES[item.baseType] || item.baseType,
            value: item.households || 0,
            baseType: item.baseType,
            sortOrder: baseTypeOrder[item.baseType as keyof typeof baseTypeOrder] || 99
          }))
                  ...
                  >
                    {wardData.map((entry, index) => (
                      <Cell
                        ...
                        fill={
                          BASE_TYPE_COLORS[
                            entry.baseType as keyof typeof BASE_TYPE_COLORS
                          ] ||
