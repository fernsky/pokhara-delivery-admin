...
interface FoodCropPieChartProps {
  pieChartData: Array<{
    name: string;
    value: number;
    percentage: string;
...
export default function FoodCropPieChart({
  pieChartData,
  FOOD_CROP_TYPES,
  FOOD_CROP_COLORS,
  dataType,
      ...
      <PieChart>
        ...
        >
          {pieChartData.map((entry, index) => {
            // Find the original food crop type key for color mapping
            const cropKey =
              Object.entries(FOOD_CROP_TYPES).find(
            ...
            return (
              <Cell
                key={`cell-${index}`}
                fill={
                  FOOD_CROP_COLORS[cropKey as keyof typeof FOOD_CROP_COLORS] ||
