...
// Update a farm
export const updateFarm = protectedProcedure
  .input(farmUpdateSchema)
  .mutation(async ({ ctx, input }) => {
    // Check permissions
    if (ctx.user.role !== "superadmin") {
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: "Only administrators can update farms",
      });
    }
    ...
    try {
      ...
      const existing = await ctx.db
        .select({ id: farm.id, slug: farm.slug })
        .from(farm)
        ...
        while (slugExists) {
          const existingSlug = await ctx.db
            ...
            .where(
              and(
                eq(farm.slug, slug),
                ne(farm.id, input.id), // Don't match our own record
              ),
            )
            .limit(1);
      ...
      const wasVerifiedBefore = (await ctx.db
        .select({ isVerified: farm.isVerified })
        .from(farm)
        .where(eq(farm.id, input.id))
      ...
      const updateData: any = {
        ...
        hasCooperativeMembership: input.hasCooperativeMembership,
        ...
        hasFarmEquipment: input.hasFarmEquipment,
        equipmentDetails: input.equipmentDetails,
        hasElectricity: input.hasElectricity,
        hasRoadAccess: input.hasRoadAccess,
        roadAccessType: input.roadAccessType,

        // Sustainability and practices
        usesChemicalFertilizer: input.usesChemicalFertilizer,
        usesPesticides: input.usesPesticides,
        usesOrganicMethods: input.usesOrganicMethods,
        ...
        // Technical support and training
        receivesExtensionServices: input.receivesExtensionServices,
