...
// Get parking facility by slug
export const getParkingFacilityBySlug = publicProcedure
  .input(z.string())
  .query(async ({ ctx, input }) => {
    try {
      const facilityData = await ctx.db
        ...
        .where(eq(parkingFacility.slug, input))
        .limit(1);
      ...
      console.error("Error fetching parking facility by slug:", error);
