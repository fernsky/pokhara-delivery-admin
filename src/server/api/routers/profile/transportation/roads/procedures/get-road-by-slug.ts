...
// Get road by slug
export const getRoadBySlug = publicProcedure
  .input(z.string())
  .query(async ({ ctx, input }) => {
    try {
      ...
      const mediaData = await ctx.db
        ...
        .where(
          and(
            eq(entityMedia.entityId, processedRoad.id),
      ...
      console.error("Error fetching road by slug:", error);
