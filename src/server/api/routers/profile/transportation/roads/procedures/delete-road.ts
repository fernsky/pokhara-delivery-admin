...
// Delete a road
export const deleteRoad = protectedProcedure
  .input(z.string())
  .mutation(async ({ ctx, input }) => {
    ...
    if (ctx.user.role !== "superadmin") {
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: "Only administrators can delete roads",
      });
    }

    try {
      // Get all media associated with this road
      ...
      // Delete the road
      await ctx.db.delete(road).where(eq(road.id, input));
