...
export const getPublicTransportById = publicProcedure
  .input(z.string())
  .query(async ({ ctx, input }) => {
    try {
      const transportData = await ctx.db
        ...
        .where(eq(publicTransport.id, input))
        .limit(1);
      ...
      //   const relatedRoads: Road[] = [];
      //   const relatedParkingFacilities: ParkingFacility[] = [];

      if (processedTransport.servingRoadIds) {
        const roadIds = processedTransport.servingRoadIds
          .split(",")
          .map((id) => id.trim());
        // Implementation for fetching related roads would go here
      }

      if (processedTransport.parkingFacilityIds) {
        const facilityIds = processedTransport.parkingFacilityIds
          .split(",")
          .map((id) => id.trim());
        // Implementation for fetching related parking facilities would go here
      }
